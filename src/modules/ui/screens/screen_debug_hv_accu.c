 ...
 * @brief       Debug screen for high voltage accumulator
 *
 * @details     
 *
...
static void build_header(lv_obj_t *scr)
{
    ...
    lv_label_set_text(title, "DBG HV ACCU");
...
static void build_bars(lv_obj_t *scr)
{
    ...
    lv_obj_set_size(bar_hv_accu_volt, 300, 20);
    lv_obj_align(bar_hv_accu_volt, LV_ALIGN_TOP_MID, -60, 80);
    ...
    lv_obj_t *lbl_bar_hv_accu_volt_value = ui_unit_label_create(scr,
    ...
    ui_unit_label_bind_value(lbl_bar_hv_accu_volt_value, &ui_subj_voltage_accu_hv, "%d");
    ...
    lv_obj_align(bar_hv_accu_temp, LV_ALIGN_TOP_MID, -60, 130);
...
lv_obj_t *screen_debug_hv_accu_create(void)
{
    /* ── Screen base ─────────────────────────────────────────────────────── */
