 ...
 *
 * One row for the low-voltage battery voltage, in a single full-width column.
 *
...
static void build_bars(lv_obj_t *scr)
{
    ...
    build_bar_row(col, &ui_sig_lv_accu_voltage, ui_sig_lv_accu_voltage.label);
}
...
lv_obj_t *screen_debug_lv_accu_create(lv_subject_t *status_subjects)
{
    ...
    ui_header_create(scr, "DBG LV ACCU", status_subjects);
    build_bars(scr);
