 ...
 *
 *              Header and page bar are added up in pixels, the hint bar is a
 *              fixed height of its own (UI_HINTBAR_H); nothing else in the
 *              screen frame is ever anything but one of these three.
 *
 ...
 *
 * The same arithmetic LVGL applies to lv_pct(UI_LAYOUT_HEADER_PCT) in
 * ui_header_create(), so the two agree to the pixel.
 *
 * @return The header height.
 */
static int32_t header_h(void)
{
    int32_t ver = lv_display_get_vertical_resolution(lv_display_get_default());

    return (ver * UI_LAYOUT_HEADER_PCT) / 100;
}
...
lv_obj_t *ui_layout_content_create(lv_obj_t *scr)
{
    ...
    lv_obj_set_style_pad_top(content, header_h() + UI_LAYOUT_PAGEBAR_H, 0);
    lv_obj_set_style_pad_bottom(content, UI_HINTBAR_H, 0);
